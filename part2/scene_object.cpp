...
bool UnitSphere::intersect( Ray3D& ray, const Matrix4x4& worldToModel,
	...
	double t_value, t_1, t_2;

	double A = rayDir.dot(rayDir);
	double B = (origin-sphereOrigin).dot(rayDir);
	double C = (origin-sphereOrigin).dot(origin-sphereOrigin) - 1;
	double D = B*B-A*C;
	//check validity of descriminant
	//calculate t value
	if (D<0){
		return false;
	}else{
	...
	//set the ray
	Point3D intersectionPoint = origin + t_value * rayDir;
...
bool UnitCylinder::intersect(Ray3D& ray, const Matrix4x4& worldToModel,
	...
	if (p2[1] > 1 || p2[1] < -1)
		t2 = -1;
	//the intersection with the plane y=1
	...
	//check for the intersection with the plane y=-1
	Vector3D normal4 = Vector3D(0, -1, 0);
	double t4 = (Vector3D(0, -1, 0).dot(normal4) + ray_obj.origin[1])/ ray_obj.dir.dot(normal4);
